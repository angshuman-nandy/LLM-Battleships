...
class OllamaWrapper(LLMWrapper):
    """LLM wrapper for locally-hosted Ollama models.

    Ollama exposes an OpenAI-compatible ``/v1/chat/completions`` endpoint, so
    this wrapper reuses the OpenAI SDK pointed at the user-supplied base URL.

    Only tool-capable Ollama models work correctly (e.g. ``llama3.1``,
    ``mistral-nemo``).  If the model returns a response without any tool_calls
    a :class:`~backend.llm.base.LLMCapabilityError` is raised immediately so
    the caller can surface a clear error to the user rather than retrying
    endlessly.
    """

    def __init__(self, config: LLMConfig) -> None:
        self._config = config
        base_url = config.endpoint_url or "http://localhost:11434/v1"
        # Ollama does not require a real API key; fall back to the placeholder
        # string "ollama" when the caller does not supply one.
        api_key = (
    ...
    # ------------------------------------------------------------------
    # Tool format conversion (identical to OpenAIWrapper)
    # ------------------------------------------------------------------
    ...
    async def place_ships(
        ...
        """Ask the Ollama model to place all ships on an empty board.

        Uses forced tool use (OpenAI-compatible) so the response is always
        structured JSON.
        ...
        Raises:
            LLMCapabilityError: If the model does not return tool_calls,
                indicating it lacks tool-use support.
        ...
        if not message.tool_calls:
            raise LLMCapabilityError(
                ...
                "Please use a tool-capable model like llama3.1 or mistral-nemo."
            )
    ...
    async def choose_shot(
        ...
        """Ask the Ollama model to pick a cell to fire at, with up to 3 retries.

        On the very first API call, if the model returns no tool_calls, a
        :class:`LLMCapabilityError` is raised immediately — this is a clear
        signal that the model does not support tool use and no amount of
        retrying will fix it.

        On subsequent retries, invalid coordinates (out-of-bounds or already
        fired) are fed back into the conversation so the model can self-correct.
        ...
        Raises:
            LLMCapabilityError: If the first response contains no tool_calls.
        ...
        for attempt in range(max_retries):
            ...
            # On the very first attempt, absence of tool_calls means the model
            # fundamentally does not support tool use — raise immediately.
            if attempt == 0 and not assistant_message.tool_calls:
                raise LLMCapabilityError(
                    "Model does not support tool use. "
                    "Please use a tool-capable model like llama3.1 or mistral-nemo."
                )
        ...
        raise ValueError(
            f"Ollama model failed to produce valid shot coordinates after "
